...
def get_input(prompt_text, allow_negative=False):
    """Safely retrieves floating point input from CLI."""
    while True:
        try:
            val_str = input(prompt_text)
            if not val_str: return None
            val = float(val_str)
            # Only enforce positive check if allow_negative is False
...
def main():
    ...
    Uc_in = get_input("Current Vel (Uc)[m/s]: ", allow_negative=True)
